...
START_TEST(calculation) {
#line 59
  Stack_t stack;
  Stack_t *st = &stack;

  char *input = (char *)calloc(S, sizeof(char));

  snprintf(input, S, "%s", "2 + 2 =");
  parser(input, st);
  ck_assert_float_eq(4, st->num[st->n - 1]);

  snprintf(input, S, "%s", "sin ( 10 ) =");
  parser(input, st);
  ck_assert_float_eq(-0.5440211109, st->num[st->n - 1]);

  snprintf(input, S, "%s", "cos ( 10 ) =");
  parser(input, st);
  ck_assert_float_eq(-0.8390715291, st->num[st->n - 1]);

  snprintf(input, S, "%s", "16 mod 4 =");
  parser(input, st);
  ck_assert_float_eq(0, st->num[st->n - 1]);

  snprintf(input, S, "%s",
           "9 mod 4 + sin ( 10 ) - tan ( 1 ) - asin ( 0.5 ) * acos ( 0.67 ) / "
           "atan ( 47.6 ) =");
  parser(input, st);
  ck_assert_float_eq(-1.3840709515, st->num[st->n - 1]);

  snprintf(input, S, "%s",
           "-2 + ( 3.3 - 1 ) * 4 ^ 2.7 - ( 2 + 2 ) * 2 / sqrt ( ln ( 144 ) ) - "
           "log ( 10 ) * ln ( 10 ) =");
  parser(input, st);
  ck_assert_float_eq(89.2246393042, st->num[st->n - 1]);

  snprintf(input, S, "%s", "log ( 10 ) =");
  parser(input, st);
  ck_assert_float_eq(1, st->num[st->n - 1]);

  snprintf(input, S, "%s",
           "12 + ( -3 + 2 ^ ( 8 * 23 - 4 ) - 5 * ( 67 / 4.6 ) ) =");
  parser(input, st);
  ck_assert_float_eq(
      1532495540865888858358347027150309183618739122183602176.0000000000,
      st->num[st->n - 1]);

  snprintf(input, S, "%s", "12 + sin ( 5 ) - 2 =");
  parser(input, st);
  ck_assert_float_eq(
      9.0410757253368615311068455938440060266475384560353982218683275457,
      st->num[st->n - 1]);

  snprintf(input, S, "%s", "sin ( sin ( 5 ) ) =");
  parser(input, st);
  ck_assert_float_eq(-0.818574144461719269169636, st->num[st->n - 1]);

  snprintf(input, S, "%s", "sin ( 90 ) =");
  parser(input, st);
...
int main(void) {
  ...
  tcase_add_test(tc1_1, simple_stack_tests);
  tcase_add_test(tc1_1, calculation);
